...
from mot_neural_solver.data.multi_partite_graph import MultiGraph
from mot_neural_solver.tracker.postprocessing import Postprocessor
from mot_neural_solver.tracker.projectors import ExactProjector, GreedyProjector
...
class MPNTracker:
    ...
    def _predict_edges(self, subgraph):
        ...
        if hasattr(subgraph, "edge_index_bb-bb"):
            joints_per_bb = (
                subgraph["edge_index_joint-joint"].shape[1]
                // subgraph["edge_index_bb-bb"].shape[1]
            )

            subgraph["edge_index_bb-bb"] = subgraph["edge_index_bb-bb"].T[knn_mask].T
            subgraph["edge_attr_bb-bb"] = subgraph["edge_attr_bb-bb"][knn_mask]

            joint_knn_mask = (
                knn_mask.reshape((-1, 1)).repeat((1, joints_per_bb)).reshape((-1))
            )

            subgraph["edge_index_joint-joint"] = (
                subgraph["edge_index_joint-joint"].T[joint_knn_mask].T
            )
            subgraph["edge_attr_joint-joint"] = subgraph["edge_attr_joint-joint"][
                joint_knn_mask
            ]
    ...
    def _evaluate_graph_in_batches(self, subseq_graph, frames_per_graph):
        ...
        for eval_round, (start_frame, end_frame) in enumerate(
            ...
            if self.dataset_params["combined_graph"]:
                ...
                joint_mask = (
                    nodes_mask.reshape((-1, 1)).repeat((1, joints_per_bb)).reshape((-1))
                )

                xs["bb"] = subseq_graph.graph_obj.x[nodes_mask]
                edge_attrs["bb-bb"] = subseq_graph.graph_obj.edge_attr[edges_mask]
                edge_indices["bb-bb"] = (
                    subseq_graph.graph_obj.edge_index.T[edges_mask].T
                    - node_names[nodes_mask][0]
                )

                joint_names = torch.arange(subseq_graph.graph_obj.x_joint.shape[0])
                ...
                edge_indices["bb-joint"][0] = (
                    edge_indices["bb-joint"][0] - node_names[nodes_mask][0]
                )
                edge_indices["bb-joint"][1] = (
                    edge_indices["bb-joint"][1] - joint_names[joint_mask][0]
                )

                edge_attrs["bb-joint"] = subseq_graph.graph_obj["edge_attr_bb-joint"][
                    joint_mask
                ]

                joint_edge_mask = (
                    edges_mask.reshape((-1, 1)).repeat((1, joints_per_bb)).reshape((-1))
                )

                if (
                    joint_edge_mask.shape[0]
                    != subseq_graph.graph_obj["edge_index_joint-joint"].shape[1]
                ):
                    print("Oh no")

                edge_indices["joint-joint"] = (
                    subseq_graph.graph_obj["edge_index_joint-joint"][:, joint_edge_mask]
                    - joint_names[joint_mask][0]
                )
                edge_attrs["joint-joint"] = subseq_graph.graph_obj[
                    "edge_attr_joint-joint"
                ][joint_edge_mask]
