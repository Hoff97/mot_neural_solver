...
class MOTSeqProcessor:
    ...
    def _store_joints(self, joints_path):
        ...
        if osp.exists(joints_path):
            print(
                "Found existing stored joint detectiond. Deleting them and replacing them for new ones"
            )
    ...
    def _keypointrcnn_resnet50_detect_and_store(self, save_dir):
        ...
        for frame, bb_boxes, frame_path, frame_ix, ids, det_ids in tqdm(loader):
            ...
            if (
                ...
                coords = torch.cat((starts, ends), dim=2).reshape((-1, 4))
                with_batch = torch.zeros(
                    (coords.shape[0], 5)
                )  # Has format (batch,x_1,y_1,x_2,y_2)
                with_batch[:, 1:] = coords
                with_batch[:, 0] = 0
                ...
                with_batch[:, 1] = with_batch[:, 1] / scaling_x
                with_batch[:, 3] = with_batch[:, 3] / scaling_x

                with_batch[:, 2] = with_batch[:, 2] / scaling_y
                with_batch[:, 4] = with_batch[:, 4] / scaling_y

                res_width = math.floor(box_size * 2 / scaling_x)
                res_height = math.floor(box_size * 2 / scaling_y)

                with_batch = with_batch.cuda()

                selected = torchvision.ops.roi_align(
                    feature_maps, with_batch, (res_height, res_width)
                )

                pooled = avg_pool(selected).reshape((num_boxes, 17, -1)).cpu()

                frame_joints_features_path = osp.join(
                    storage_path, f"{frame_ix.item()}_features.pt"
                )

                torch.save(pooled, frame_joints_features_path)
    ...
    def _mmpose_top_down_model_detect_and_store(self, model_name, save_heatmaps=False):
        ...
        ds = MMPOSECompatibleDataset(
            self.det_df,
            self.det_df.seq_info_dict,
            model_in_size,
            debug=self.dataset_params["debug"],
        )
    ...
    def load_or_process_detections(self):
        ...
        joints_path = osp.join(
            seq_path, "processed_data", "joints", det_file_to_use, self.keypoint_model
        )
        ...
        if not joints_ok or self.dataset_params["overwrite_joints"]:
            ...
            print(
                f"Loading processed joints for sequence {self.seq_name} from {joints_path}"
            )
